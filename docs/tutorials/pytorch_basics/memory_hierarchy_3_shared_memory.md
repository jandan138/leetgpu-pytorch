...
1.  **搬运阶段（做苦力）**：
    *   Thread 1 **并不搬运** `A[1][k]`（它自己要用的数）。
    *   Thread 1 可能会去搬运 `A[0][1]`（Thread 0 要用的数）。
    *   为什么？因为 Thread 0 搬 `A[0][0]`，Thread 1 搬 `A[0][1]`... 这样凑在一起，刚好是**连续**的 32 个数。显存控制器最喜欢！

2.  **计算阶段（享成果）**：
    *   Thread 1 需要 `A[1][k]`。这个数是谁搬回来的？
    *   可能是 Thread 32 搬回来的（因为 Thread 32 负责搬第二行的头）。
    *   Thread 1 直接从 **Shared Memory** 里拿这个数。
    *   **好处**：虽然 Thread 1 没有亲自去搬自己要的数，但它帮别人搬了顺手的数，别人也帮它搬了它要的数。大家的总时间缩短了。
