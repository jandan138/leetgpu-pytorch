...
让我们深入看一看矩阵乘法 $C = A \times B$ 的计算过程。假设矩阵大小都是 $N \times N$。

#### 笨办法 (Naive Implementation)
每个线程负责计算 $C$ 中的一个元素 $C_{ij}$。
公式是：$C_{ij} = \sum_{k=0}^{N-1} A_{ik} \times B_{kj}$

*   **动作**：为了算这就**这一个点**，线程必须从 Global Memory 读取 $A$ 的整整一行 ($N$ 个数) 和 $B$ 的整整一列 ($N$ 个数)。
*   **总读取量**：$C$ 共有 $N^2$ 个点。所以总读取次数 = $N^2 \times (2N) = 2N^3$。
*   **问题**：$A$ 中的同一个元素（比如 $A_{00}$）会被 $B$ 的第 0 列的所有元素用到。但在笨办法里，每次用到它都要去遥远的 Global Memory 读一次。**没有复用！**

---

### 2.2 聪明办法：Tiling (分块) + Shared Memory

我们把大矩阵切成很多小块 (Tile)，比如 $32 \times 32$ 的小方块。
...
2.  **计算 (Compute)**：所有线程都**只从 Shared Memory** 读取数据来计算。
    ...
    *   **复用**：$A$ 的这一小块数据，被 Block 内的所有线程反复读取了 $BLOCK\_SIZE$ 次，但只从 Global Memory 读了一次！
...
*   **笨办法读取次数**：$2N^3$
*   **Tiling 读取次数**：
    *   每个 $C$ 的 Tile ($32 \times 32$) 需要计算 $N$ 次（分 $N/32$ 个阶段）。
    *   每个阶段只需要读 $A$ 的 Tile ($32 \times 32$) 和 $B$ 的 Tile ($32 \times 32$）。
    *   总读取量 $\approx 2N^3 / 32$。
