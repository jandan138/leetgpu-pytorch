...
以具体数字为例，设 BM=BK=BN=2，M=K=N=4：

```text
完整的矩阵乘法（朴素版做法）：
C[0:2, 0:2] = A[0:2, 0:4] @ B[0:4, 0:2]
              ← A 的前两行全部 4 列        ← B 的全部 4 行前两列
              一口气算完，A 要读 2×4=8 个元素

分块版做法（把 N=4 这一维拆成两段，每段 BN=2）：
第 1 步（n_start=0）：
  C[0:2, 0:2] += A[0:2, 0:2] @ B[0:2, 0:2]
                 ← A 的左半边 tile       ← B 的上半边 tile

第 2 步（n_start=2）：
  C[0:2, 0:2] += A[0:2, 2:4] @ B[2:4, 0:2]
                 ← A 的右半边 tile       ← B 的下半边 tile

两步的结果加起来 = 朴素版一步的结果（数学上完全等价）
```
